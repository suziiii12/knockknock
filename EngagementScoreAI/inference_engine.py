...
Designed to be driven by an external backend (HTTP API or direct Python import):
  ...
  Direct Python usage:
    ...
    ...
    json_path = engine.stop()   # returns path of written JSON file
...
On stop():
  ...
  2. Calls session_export.export_session() → writes exports/session_N_TIMESTAMP.json
  3. Returns the Path to the JSON file
...
class InferenceEngine:
    ...
    Exports full score sequence to JSON when stop() is called.
    """
    ...
    def stop(self) -> Optional[Path]:
        """
        Stop the pipeline, close the DB session, and export scores to JSON.

        Returns
        -------
        Path  to the exported JSON file, or None if no scores were recorded.
        """
        ...
        if sid is not None:
            end_session(sid)
            print(f"[InferenceEngine] Session {sid} ended — exporting JSON…")
            try:
                json_path = export_session(
                    session_id=sid,
                ...
                print(f"[InferenceEngine] JSON export failed: {e}")
